...
namespace Zork\OpenId\Extension;

use ZendOpenId\Extension\AbstractExtension;
 ...
 * @codeCoverageIgnore
 */
class Ax extends AbstractExtension
{
    ...
    /**
     * Returns array of allowed AX variable names.
     *
     * @return array
     */
    ...
    public function prepareRequest( &$params )
    {
        ...
        if ( is_array( $this->_props ) && count( $this->_props ) > 0 )
        {
            ...
            $params['openid.ax.mode']   = self::MODE;
    ...
    /**
     * Adds additional AX data to OpenId 'id_res' response.
     *
     ...
     * to 'openid.ax.*' variables in response
     *
     * @param array $params response's var/val pairs
     * @return bool
     */
    ...
    /**
     * Addes AX properties that are allowed to be send to consumer to
    ...
    {
        $data[str_replace( '\\', '_', get_class() )] = $this->getProperties();
        return true;
    }

    /**
     * Check if given $data contains necessury AX properties to sutisfy
     * OpenId request. On success sets AX response properties from given
